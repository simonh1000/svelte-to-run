...
export const say = function(payload) {
    ...
    beep(1)
        .then(() => {
            ...
            // } else {
            //     let tmp = new Audio(payload.type.toLowerCase() + ".wav");
            //     tmp.play();
            // }
        })
...
export const synthesise = function(txt) {
    window.speechSynthesis.speak(new SpeechSynthesisUtterance(txt));
};

// plays x beeps
export const beepX = function(x) {
    let tmp = new Audio();
    tmp.src = beeep;
    if (x <= 0) {
        ...
        tmp.play();
        tmp.addEventListener("ended", () => beepX(x - 1), false);
    }
};
